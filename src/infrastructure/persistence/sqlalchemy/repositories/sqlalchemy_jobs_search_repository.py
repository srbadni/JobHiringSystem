...
from application.jobs_search.ports.jobs_search_repository import JobsSearchRepository, GetJobsQueries
...
class SQLAlchemyJobsSearchRepository(JobsSearchRepository):
    ...
    async def get_jobs(self, queries: GetJobsQueries) -> list[JobSearchResult]:
        ...
        if queries.keywords:
            filters.append(JobPostingORMModel.job_title.ilike(f"%{queries.keywords}%"))
        if queries.province_ids:
            filters.append(JobPostingORMModel.province_id.in_(queries.province_ids))
        if queries.job_category_ids:
            filters.append(JobPostingORMModel.job_category_id.in_(queries.job_category_ids))
        if queries.work_modes:
            filters.append(JobPostingORMModel.work_mode.in_(queries.work_modes))
        if queries.work_experiences:
            filters.append(JobPostingORMModel.work_experience.in_(queries.work_experiences))
        if queries.salary_range_ids:
            filters.append(JobPostingORMModel.salary_range_id.in_(queries.salary_range_ids))
        ...
        stmt = (
            ...
            .join(JobCategoryORMModel, JobPostingORMModel.job_category_id == JobCategoryORMModel.id)
            .join(ProvinceORMModel, JobPostingORMModel.province_id == ProvinceORMModel.id)
            .join(SalaryRangeORMModel, JobPostingORMModel.salary_range_id == SalaryRangeORMModel.id)
            .join(CityORMModel, JobPostingORMModel.city_id == CityORMModel.id)
            .where(*filters)
        )
