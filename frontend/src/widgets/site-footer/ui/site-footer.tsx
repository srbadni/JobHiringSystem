...
function FooterLinkGroup({title, links}: FooterLinkGroupProps) {
    return (
        <nav aria-label={title}>
            <Typography variant="body" className="mb-5 !font-bold text-foreground">{title}</Typography>
                ...
                {links.map((link) => (
                    <li key={link.label}>
                        <Link className="transition-colors hover:text-primary focus-visible:text-primary" href={link.href}>
                            <Typography className="text-footer-link" tone="muted">
                                {link.label}
                            </Typography>
                        </Link>
                    </li>
...
function FooterBrand() {
    return (
        <div>
            ...
            <Typography variant="body" className="mt-6 text-muted md:mt-7">
...
export function SiteFooter() {
    return (
        <footer className="mt-auto border-t-gray-200 bg-surface px-4 pb-4 pt-12 md:px-8 md:pb-5 md:pt-11">
